...
set USAGE 'usage: build_dosbox [--no-symlink] [--no-clean] [--no-pull]
...
function rebuild --argument-names arg
  ...
  ! _require_cmd "aclocal"; and return 1
  if [ ! -d ~/Source/dosbox-x ]
    echo 'build_dosbox: error: could not find DOSBox-X source directory at ~/Source/dosbox-x'
    exit 1
  end
  ...
  if [ "$status" -ne 0 ]
    echo 'build_dosbox: error: could not build DOSBox-X'
    exit 1
  end
...
#_dada_fish _register_dependency "brew" "automake" "aclocal"
#_dada_fish _register_bin regular "build_dosbox" "…" "build_dosbox.fish" "Builds a new DOSBox application bundle"
